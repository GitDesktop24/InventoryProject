// src/ProfileDropdown.jsx

import React, { useState } from 'react';

// Component to handle the modal popup for logout confirmation
...
const ProfileDropdown = ({ profile, handleSaveUsername, handleSignOut }) => {
    ...
    const startEdit = () => {
        ...
        setCurrentUsername(profile.username); // Reset input to current profile name
    };

    const saveAndClose = () => {
        handleSaveUsername(currentUsername); // Pass new username up to AdminPage
        ...
        <div style={styles.dropdownContainer}>
            ...
            {isModalOpen && (
                <LogoutConfirmationModal 
                    onConfirm={() => {
                        handleSignOut(); // Trigger sign out in AdminPage
...
const styles = {
    dropdownContainer: {
        position: 'relative',
        zIndex: 900, // Ensure it sits above main content
        marginRight: '20px',
    },
    dropdownToggle: {
        padding: '8px 15px',
...
const modalStyles = {
    // ... (Modal styles remain the same, ensure they are defined in AdminPage scope or imported/defined here)
    backdrop: {
        position: 'fixed',
        top: 0,
        left: 0,
